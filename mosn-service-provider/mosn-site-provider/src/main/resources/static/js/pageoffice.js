...
var POBrowser = {
    ...
    showInstallDlg: function() {
        if (confirm("您需要安装PageOffice来打开文档。现在立即安装PageOffice吗？\r\n注意：安装完成后，请重新访问当前页面。")) {
            window.location.href = this.getRootPath().replace('/js','') + "/posetup.exe"
        }
    },
    openWindow: function(strURL, strOptions, strArgument) {
        ...
        zsxmlhttp.onload = function() {
            if (zsxmlhttp.responseText.indexOf('"name":"jsonx"') > 0) {
                bPOIsInstalled = true;
                po_ajax({
                    url: POBrowser.getRootPath().replace('/js','') + "/poserver.zz",
                    type: "POST",
...
POBrowser.addCssByLink(POBrowser.getRootPath().replace('/js','') + "/pobstyle.css");
POBrowser.checkSSL();
var poEvent = {};
