...
public class FtpUtil {
     ...
     * @param ftpPort FTP 登录密码
     * @param ftpUserName FTP登录用户名
     * @param ftpPassword FTP端口 默认为21
     * @return
     */
     ...
     * @param fileName 文件名称
     */
    public static void downloadFtpFile(String ftpHost, int ftpPort, String ftpUserName, String ftpPassword,
...
//            byte2File(input2byte, localPath, fileName);
     ...
     * @param ftpHost FTP IP地址
     * @param ftpPort FTP端口
     * @param ftpUserName TP 用户名
     * @param ftpPassword FTP用户名密码
     * @param ftpPath FTP服务器中文件所在路径 格式： ftptest/aa
     * @param fileName 文件名称
     * @return
     */
    public static byte[] downloadFtpFile(String ftpHost, int ftpPort, String ftpUserName, String ftpPassword, String ftpPath, String fileName) {
        FTPClient ftpClient = null;
        ...
        try {
            ...
            String fileNameTemp = new String(fileName.getBytes(LOCAL_CHARSET), SERVER_CHARSET);
            ftpClient.changeWorkingDirectory(ftpPath);
            retrieveFileStream = ftpClient.retrieveFileStream(fileNameTemp);

            return input2byte(retrieveFileStream);
        } catch (FileNotFoundException e) {
            logger.error("没有找到" + ftpPath + "文件");
            e.printStackTrace();
        } catch (SocketException e) {
     ...
     * @param filePath FTP服务器文件存放路径。例如分日期存放：/2022/03/14 文件的路径为basePath+filePath
